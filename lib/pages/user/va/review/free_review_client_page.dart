...
import 'package:letter_a/pages/user/va/review/sent_review_client_alert.dart';
...
class ReviewClientPage extends StatefulWidget {
  ...
  final String? ClientPhotoProfile;
  ReviewClientPage({
    required this.aplicationId,
    required this.clientId,
    required this.clientName,
    required this.contract,
    required this.ClientPhotoProfile,
  });
...
class _ReviewClientPageState extends State<ReviewClientPage> {
  ...
  Future<void> giveRatingForVA() async {
    ...
    final response = await http.post(
      ...
      body: {
        ...
        "ratingClient": _ratingController.text,
      },
    );
    ...
    if (data['success']) {
      print('>>>>Here');
      await fetchAverageRating(int.parse('${widget.clientId}'));
  ...
  Future<Map<String, dynamic>> fetchAverageRating(int userId) async {
    final String apiUrl =
        '$server/api/v1/ratings/api_average_rating_client.php'; // Ganti dengan URL API Anda
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success']) {
          print('sukses give rating ${widget.clientId}');
          return {
          ...
          child: Column(
            children: [
              Container(
                  ...
                  child: Text("Give an assessment to your Client",
                      style: LText.H3())),
              SizedBox(
                height: 24,
              ),
                            ...
                            children: [
                              ClipRRect(
                                  ...
                                  child: SizedBox(
                                      ...
                                      width: 40,
                                      child: Image.network(
                                          '$server/api/v1/${widget.ClientPhotoProfile}',
                                          width: 40,
                                          height: 40,
                                          fit: BoxFit.cover))),
                              SizedBox(width: 12),
